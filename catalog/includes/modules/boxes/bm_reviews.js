...
*/
function bm_reviews() // CLASS
{
    this.code = 'bm_reviews';
    this.group = 'boxes';
    this.title;
    this.description;
    this.sort_order;
    ...
    this.check = function()
    {
        return typeof 'MODULE_BOXES_REVIEWS_STATUS' != 'undefined'
    }
    ...
    this.execute = function(oscTemplate)
    {
        ...
        if (random_product) {
            ...
            rand_review_query = tep_db_query('select substring(reviews_text, 1, 60) as reviews_text from ' + TABLE_REVIEWS_DESCRIPTION + ' where reviews_id = \'' + random_product['reviews_id'] + '\' and languages_id = \'' + languages_id + '\'');
            rand_review = tep_db_fetch_array(rand_review_query);
            rand_review_text = tep_break_string(tep_output_string_protected(rand_review[0]['reviews_text']), 15, '-<br />');
            reviews_box_contents += '<div class="ui-widget-content infoBoxContents"><div align="center">'
                                  + '<a href="' 
                                  + tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' + random_product['products_id'] + '&reviews_id=' + random_product['reviews_id']) 
                                  + '">' 
                                  ...
                                  + '</div></div>';
        } else if (HTTP_GET_VARS['products_id']) {
    ...
    this.install = function()
    {
        ...
        + TABLE_CONFIGURATION 
        + ' (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values (\'Enable Reviews Module\', \'MODULE_BOXES_REVIEWS_STATUS\', \'True\', \'Do you want to add the module to your shop?\', \'6\', \'1\', \'tep_cfg_select_option(array(\\\'True\\\', \\\'False\\\'), \', now())');
        tep_db_query('insert into ' 
        + TABLE_CONFIGURATION 
        ...
        tep_db_query('insert into ' 
        + TABLE_CONFIGURATION 
        + ' (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values (\'Sort Order\', \'MODULE_BOXES_REVIEWS_SORT_ORDER\', \'0\', \'Sort order of display. Lowest is displayed first.\', \'6\', \'0\', now())');
    }
